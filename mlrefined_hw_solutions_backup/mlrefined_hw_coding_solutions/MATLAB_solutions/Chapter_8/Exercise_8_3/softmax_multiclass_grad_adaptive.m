function softmax_multiclass_grad_adaptive()
...
[W, obj] = softmax_multiclass_grad(X,y,W0);
...
%%%%%%%%%%%%%%%%% subfunctions %%%%%%%%%%%%%%%%%%%
function [W, obj] = softmax_multiclass_grad(X,y,W0)
    ...
    while k <= max_its
        ...
        grad = zeros(size(W));
        for c = 1:C
            w_temp = W - repmat(W(:,c),1,C);
            d = 1./(sum(exp(X'*w_temp),2));
            ind = find(y == c);
            omega = zeros(P,1);
            omega(ind) = 1;
            d = d - omega;
        ...
        alpha = adaptive_step(W,grad,obj(k),X,y,C);
        W = W - alpha*grad;
        
        % update counter
        obj = [obj, objective(X,y,W,C)];
        k = k + 1;
        
    end
end

function p = adaptive_step(W,g,o,Z,y,C)
    g_n = norm(g,'fro')^2;
    step_l = 1;
    step_r = 0;
    u = 1;
    ...
    while step_l > step_r && u < 20
        ...
        % left
        step_l = objective(Z,y,W - p*g,C) - o;
...
function obj = objective(X,y,W,C)
    obj = 0;
    for c = 1:C
        w_temp = W - repmat(W(:,c),1,C);
        ind = find(y == c);
        X_temp = X(:,ind);
        d = log(sum(exp(X_temp'*w_temp),2));
        obj = obj + sum(d);
    end
end
